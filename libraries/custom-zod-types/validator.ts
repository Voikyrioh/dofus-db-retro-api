 ...
 * ZodError escape), so the client receives `invalid-payload` + per-field details,
 * and any raw ZodError reaching the handler is treated as a domain error.
 */
export function betterZodValidator<T extends z.ZodSchema, Target extends keyof ValidationTargets>(target: Target, schema: T) {
