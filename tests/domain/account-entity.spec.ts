...
describe('AccountEntity', () => {
	describe('constructor', () => {
		...
		it('should throw when id is not a valid ULID', () => {
			expect(
				() => new AccountEntity(validAccount({ id: 'not-a-ulid' })),
			).to.throw()
		})
	})
	...
	describe('CreateAccount', () => {
		it('should resolve to an AccountEntity instance', async () => {
			const entity = await AccountEntity.CreateAccount(
				'testuser',
				'password123',
				'test@example.com',
			)
	...
	describe('changePassword', () => {
		it('should update passwordHash to the new value', () => {
			const entity = new AccountEntity(
				validAccount({ passwordHash: 'oldhash' }),
			)
	...
	describe('getInfo', () => {
		it('should return username and email', () => {
			...
			expect(entity.getInfo()).to.deep.equal({
				username: 'testuser',
				email: 'test@example.com',
			})
		})
	...
	describe('getInfoExt', () => {
		it('should return id, username, email, and role', () => {
			...
			expect(entity.getInfoExt()).to.deep.equal({
				id,
				username: 'testuser',
				email: 'test@example.com',
				role: 1,
