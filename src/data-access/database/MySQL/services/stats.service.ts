import {
	type StatsEntity,
	statsEntitySchema,
} from '../../../../domain/entities/stats.entity'
...
export function parseStats(dbStats: string): StatsEntity[] {
	...
	for (const stat of serializedStat) {
		...
		const dice = jet.match(/(?<n>\d+)d(?<v>\d+)(\+(?<f>\d+))?/)?.groups as {
			n: string
			v: string
			f?: string
		} | null
