    ...
    constructor(stats: StatsEntity[]) {
        for(const stat of stats) {
            if ( stat.id.startsWith('ADD_')) {
                this[stat.id.replace('ADD_', '').replace('2', '') as keyof Statistics] = {min: stat.min, max: stat.max};
            } else if ( stat.id.startsWith('REM_')) {
                this[stat.id.replace('REM_', '').replace('2', '') as keyof Statistics] = {min: stat.min * -1, max: stat.max * -1};
            } else {
